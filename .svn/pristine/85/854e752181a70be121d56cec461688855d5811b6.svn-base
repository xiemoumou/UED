...
//文章发布
app.controller('writeCtrl',["$scope","$state","$compile","$http",function($scope,$state,$compile,$http){
    
    UE.delEditor("f-editor");//删除原来视图
    var ue = UE.getEditor('f-editor',{//初始化编辑器大小
        initialFrameWidth: '100%',
        initialFrameHeight:683
    });

    //添加标签 jquery方法
    $('.f-label ul li a').each(function() {
        var str = "<span style='padding:5px; background:#F1F3F1'>";
        $(this).on('click', function(e) {
            $('.f-placeholer').remove();
            if($('.input-lable span').length>"4"){
                return;
            }
            var strnew = $(this).html() + '<i>×</i></span>';
            strnew = str + strnew;
            $('.input-lable').append(strnew + ' ');  
        });
    });
    $('.input-lable').click(function(event) {
