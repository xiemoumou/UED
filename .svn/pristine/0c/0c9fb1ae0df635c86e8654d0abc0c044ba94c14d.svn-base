...
//登录注册弹窗自定义指令
app.directive('loginReg',function($state,$compile,$http){
    return {
        scope:{
            loginUrl:'=',//登录地址
            registerUrl:'=',//注册地址
        ...
        restrict : 'EAC',
        replace : true,
        template : '<ul class="news-nav pull-right f-mt15"> <li> <a href="#" user-state="state" rel="category" id="js_login" > <span class="glyphicon glyphicon-map-marker"> </span>登录 </a>  <a href="#" user-state="state" rel="category" id="js_register" ><span class="glyphicon glyphicon-italic-line"> </span>注册</a> </li></ul>',
...
app.controller('newsCtrl1111111111111111111',["$scope","$state","$compile","$http",function($scope,$state,$compile,$http){
    /**
     * Created by 高月娇 on 2017/3/26.
     */
    /*运动函数*/
    ...
    function popupFn(){
        ...
        str="<div class='sign' id='sign'><div class='part login-part' id='js_login_part'>"
            +"<form action=''>"
            +"<div class='switch' id='js_register_part'>切换注册</div>"
            +" <h3>登录 <span  ng-class='errClass' ng-bind='errMsg' ></span></h3>"
            +"<p>"
            +"<label for='uesrname' class='login-info'>"
            +"<span class='glyphicon glyphicon-user'></span>"
            ...
            +"<div class='part register-part' id='js_register'>"
            +"<form action=''>"
            +"<div id='login-active' class='switch'>切换登录</div>"
            +" <h3>注册 <span  ng-class='errClass1' ng-bind='errMsg1' ></span></h3>"
            +"<p><label for='uesrname' class='login-info'><span class='glyphicon glyphicon-user'></span></label>"
            +"<input type='text'  ng-class='Class.uName' placeholder='请输入用户名' ng-model='register.uName' ng-blur='Test(Msg.uName,2)'></p>"
            +"<p><label for='password' class='login-info'><span class='glyphicon glyphicon-lock'></span></label>"
